...
- **`triptych`** (`conductors/ws/triptych/`) — pressure-gated thirds (built from the skeleton-lab research on branch `claude/code-skeleton-extraction-rv0ovo`): inert until the visible window first crosses the 90% high-water mark, then divides the context into a raw bottom band (the most recent ~⅓ of the cap), a middle band where TypeScript, JavaScript, and Python discovered directly in assistant text, thinking, and tool-result content become tagged, recoverable **tree-sitter L2 skeleton** `replace` folds (signatures kept, bodies elided; a decline-to-fold gate skips blocks a skeleton would not actually shrink), and a top band swept into one **lossy** summary group using compaction-naive's `COMPACTION_SYSTEM` verbatim (untagged, recursive — the agent has no path back; human detach recovers). Discovery never relies on tool names, arguments, paths, or extensions; user/system/tool-call blocks are ignored. Locks `human-steering`/`agent-unfold` (fully exclusive; `recall` stays available by contract). **Runs out of process** like thermocline: `runner.mjs` imports the committed `triptych-sdk.mjs` (same regenerate script as above) and injects the wasm skeleton engine (`skeleton.mjs` — needs a one-time `npm install` in `conductors/ws/triptych/`; the spawn fails loudly without it). `TriptychConductor` extends `AgedSummaryConductor` via protected band/prompt hooks.
