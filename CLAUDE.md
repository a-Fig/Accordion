...
- `core/protocol.ts` — the v20 wire messages (`hello`/`snapshot`/`event`/`telemetry`/`commandResult`/`stream`/`conductorState`/`conductorStatus`/`wireDeparting`/`turnCommitted`/`proposeResult`/`completeResult`/`controller`/`notice` server→client; `command`/`resnapshot`/`propose`/`completeRequest`/`setConductorStatus`/`holdRelease`/`cancelComplete`/`claimController` client→server), `WireBlock`, `FoldOp`/`GroupOp`, `PROTOCOL_VERSION`, `DOOR_PORT`, and the `sanitizeCommand`/`sanitizeOps`/`sanitizeSurfaceId`/`sanitizeSurfaceLabel` ingress validators (v20 adds the provider-receipt `calibrationThroughOrder` frontier so stale k never scales newly appended blocks; see [ADR 0025](docs/adr/0025-token-calibration.md))
