...
| path | what |
|------|------|
| `core/` | Framework-free shared model: `Truth` (canonical context state, `truth.ts`), the v20 wire protocol (`protocol.ts`), replica (de)serialization (`replica.ts`), agent unfold/recall resolution (`agentView.ts`). Imported by the extension (relative `../core`) and the app (`$core` alias) — no Svelte, no Node-only deps |
...
| `app/src/lib/live/` | WS client (replica + remote control over protocol v20), session discovery, CC transcript browsing |
...
- `Truth.calibration`/`calibrationBase` — host-set, rev-stamped provider calibration using `real = base + scale·est` (issue #102). `calTotalTokens(n)` applies the base once to whole-request totals; `calTokens(n)` applies only the scale to per-block/component deltas, so newly appended tool results do not inherit fixed prompt/schema overhead. `protectedFromIndex()` and conductor component reads use the calibrated scale, while `stats()` reports affine-calibrated `liveTokens`/`fullTokens`; `budget`/`protectTokens`/`contextWindow` remain literal real-token dials. See [ADR 0025](docs/adr/0025-token-calibration.md)
...
- `core/protocol.ts` — the v20 wire messages (`hello`/`snapshot`/`event`/`telemetry`/`commandResult`/`stream`/`conductorState`/`conductorStatus`/`wireDeparting`/`turnCommitted`/`proposeResult`/`completeResult`/`controller`/`notice` server→client; `command`/`resnapshot`/`propose`/`completeRequest`/`setConductorStatus`/`holdRelease`/`cancelComplete`/`claimController` client→server), `WireBlock`, `FoldOp`/`GroupOp`, `PROTOCOL_VERSION`, `DOOR_PORT`, and the ingress validators. v20 adds the affine `calibrationBase` snapshot/config field for issue #102.
