...
| [`handoff/`](handoff/) | Simulates the hard reset — write a handoff doc, `/clear`, reseed a fresh agent. Owns a zero inherited tail (`tail-size` lock) and folds the whole current session into one handoff. The sibling foil to naive compaction. |
...
| [`handoff/`](handoff/) | TypeScript | in-process | **Fresh-start-with-a-handoff simulation.** Simulates the *other* long-session move — not `/compact`, but `/clear` + reseed: write a handoff document via `host.complete` and hand it to a cold agent that has nothing else. Same single-`group`-with-LLM-digest shape and visible-window hysteresis as naive compaction, but it also holds the **`tail-size`** lock with `tailTokens = 0`, so it owns no inherited old-session tail and folds the whole current conversation into one handoff — a hard-reset sawtooth, not a gentle curve. Lossy + recursive (each handoff sees only the prior handoff + new work — compounding amnesia); no `{#code FOLDED}` tag → the agent can't self-unfold, only the human can detach. The sibling foil to naive compaction. See [ADR 0017](../docs/adr/0017-handoff-conductor.md). |
